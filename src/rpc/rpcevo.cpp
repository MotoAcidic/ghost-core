...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The Ghost address to send the collateral to.\n"
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The Ghost address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            "%d. \"payoutAddress\"            (string, required) The Ghost address to use for masternode reward payments.\n"
        },
...
static void FundSpecialTx(CWallet* pwallet, CMutableTransaction& tx, const SpecialTxPayload& payload, const CTxDestination& fundDest)
{
    ...
    if (!pwallet->CreateTransaction(*locked_chain, vecSend, wtx, nFeeRequired, nChangePosRet, strError, coinControl, false))
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    CMutableTransaction tx;
    tx.SetType(TXN_PROVIDER_UPDATE_SERVICE);
...
UniValue protx_update_registrar(const JSONRPCRequest& request)
{
    ...
    CMutableTransaction tx;
    tx.SetType(TXN_PROVIDER_UPDATE_REGISTRAR);
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    CMutableTransaction tx;
    tx.SetType(TXN_PROVIDER_UPDATE_REVOKE);
