...
static bool CheckService(const uint256& proTxHash, const ProTx& proTx, CValidationState& state)
{
    ...
    static int defaultPort = Params().GetDefaultPort();
    if (proTx.addr.GetPort() != defaultPort) {
        return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-protx-ipaddr-port");
    }
...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pindexPrev) {
        ...
        if (Params().GetConsensus().fUniqueKeys) {
            if (ptx.keyIDOwner != ptx.keyIDVoting) {
                return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-protx-key-not-same");
            }
        }
    }
...
{
    if (tx.IsEvoVersion() != TXN_PROVIDER_UPDATE_SERVICE) {
        return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-protx-type");
    }
...
{
    if (tx.IsEvoVersion() != TXN_PROVIDER_UPDATE_REGISTRAR) {
        return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-protx-type");
    }
...
{
    if (tx.IsEvoVersion() != TXN_PROVIDER_UPDATE_REVOKE) {
        return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-protx-type");
    }
