...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to ghostd). This changes the RPC endpoint used, e.g. http://127.0.0.1:51725/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
                ...
                if (!error.isNull()) {
                    ...
                    if (error.isObject())
                    {
                        ...
                        if (errCode.isNum() && errCode.get_int() == RPC_WALLET_NOT_SPECIFIED) {
                            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to ghost-cli command line.";
                        }
                    }
                } else {
                    // Result
